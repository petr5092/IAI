...
- Часть B (structured vision): выбран детекционный трек на датасете PennFudanPed с целью продемонстрировать использование предобученной детекторной модели и сравнение двух режимов инференса по порогу score.
...
- Датасет: PennFudanPed (пешеходы)
- Трек: detection
- Что считается ground truth: bounding boxes, полученные из масок PennFudanPed (пересчёт минимального ограничивающего прямоугольника для каждого сегмента).
- Какие предсказания использовались: выходы предобученной детекторной модели (`fasterrcnn_mobilenet_v3_large_fpn`) — фильтрация по `score` и по метке `person`.
- Комментарий: PennFudanPed — небольшой датасет для демонстрации локализации людей; для него разумно применять предобученные на COCO модели и оценивать поведение при разных порогах confidence.
