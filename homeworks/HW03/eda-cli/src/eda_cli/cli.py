...
def report(
    ...
    title: str = typer.Option("Report", help="Заголовок отчёта."),
    top_k_categories: int = typer.Option(5, help="Сколько топовых категорий сохранять.")
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Наличие колонок с одинаковыми значениями: **{quality_flags['has_constant_columns']}**\n")
        f.write(f"- Наличие дубликтов в user_id: **{quality_flags['has_suspicious_id_duplicates']}**\n")
        ...
        if not top_cats:
            ...
            f.write(f"См. файлы в папке `top_categories/` (top-{top_k_categories} значений для каждой колонки).\n\n")
